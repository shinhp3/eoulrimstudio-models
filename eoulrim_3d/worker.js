/**
 * Cloudflare Worker — admin에서 호출해 GitHub models/에 STL을 올리고, 목록·뷰어 URL을 돌려줍니다.
 * wrangler.toml 등에서 이 파일을 엔트리로 지정하세요.
 */
...
function requireEnv(env) {
  ...
  if (!token || !username || !repo) {
    throw Object.assign(new Error("서버 설정(GITHUB_TOKEN, GITHUB_USERNAME, GITHUB_REPO)이 비어 있습니다."), {
      status: 500,
    });
  }
...
async function handleUpload(request, env) {
  try {
    ...
    const b64 =
      body.content ?? body.base64 ?? body.data ?? body.file;
...
async function handleList(env) {
  try {
    const { token, username, repo } = requireEnv(env);
    const pathUrl = `${GITHUB_API}/repos/${username}/${repo}/contents/${gitContentsPath(MODELS_PATH)}?ref=${encodeURIComponent(DEFAULT_BRANCH)}`;
